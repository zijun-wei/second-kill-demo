...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public UserBO getUserByIdInCache(Integer id) {
        UserBO userBO = (UserBO) redisTemplate.opsForValue().get("user_validate"+id);
        if(userBO == null){
            userBO = this.getUserById(id);
            redisTemplate.opsForValue().set("user_validate"+id, userBO);
            redisTemplate.expire("user_validate"+id, 10, TimeUnit.MINUTES);
        }
        return userBO;
    }
...
//        }
    ...
    @Override
    public boolean validateAdmin(String telephone, String password) throws BusinessException {
        UserDO userDO = userDOMapper.selectByTelephone(telephone);
        if(userDO == null) {throw new BusinessException(EmBusinessError.USER_NOT_EXIST);}
        else if(userDO.getThirdPartyId().equals("admin")){
            UserPasswordDO userPasswordDO = userPasswordDOMapper.selectByUserId(userDO.getId());
            UserBO userBO = convertFromDataObjectToBusinessObject(userDO, userPasswordDO);
            if(!StringUtils.equals(password, userBO.getEncrptPassword())){
                throw new BusinessException(EmBusinessError.USER_LOGIN_FAIL);
            }else{
                return true;
            }
